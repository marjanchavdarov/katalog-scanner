...
export default function ProfileScreen() {
  ...
  async function loadUser() {
    ...
    try {
      const r = await fetch('https://botapp-u7qa.onrender.com/api/chains');
      const data = await r.json();
      setStats({ saved: saved.length, scanned: history.length, lists: lists.length, stores: data.count || 0 });
    } catch {
      setStats({ saved: saved.length, scanned: history.length, lists: lists.length, stores: 25 });
    }
  }
